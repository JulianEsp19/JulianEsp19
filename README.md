# 👨‍💻 Jesús Julián Esparza Rosas

**Desarrollador de Software Junior | Backend & Frontend | Java · PHP · Python**

📍 Guadalajara, Jalisco, México  
📧 jesusjulianesparza@gmail.com  
📱 33-23-39-61-48  
🌐 [GitHub - JulianEsp19](https://github.com/JulianEsp19)

---

## 🚀 Sobre mí

Soy un desarrollador de software en formación con experiencia en **proyectos web, móviles y de programación paralela**.  
Me motiva el aprendizaje continuo, la resolución de problemas y la creación de soluciones prácticas.  

Busco integrarme a un equipo de desarrollo para fortalecer mis habilidades en **backend**, sin dejar de crecer en **frontend**.

---

## 🛠️ Tecnologías y herramientas

| Categoría | Tecnologías |
|------------|-------------|
| **Lenguajes** | Java · PHP · Python · SQL · HTML · CSS · JavaScript |
| **Frameworks / Librerías** | Firebase |
| **Bases de datos** | MySQL · SQLite |
| **Entornos** | Android Studio · VS Code · NetBeans · PyCharm |
| **Herramientas** | Git · GitHub · XAMPP |

---

## 💼 Proyectos destacados

### 🛒 E-commerce con PHP y XAMPP
Sistema de comercio electrónico con gestión de usuarios, productos y generación de pedidos en PDF enviados por correo electrónico.  
**Tecnologías:** PHP, SQL, XAMPP  
📂 [Ver proyecto](./projects/ecommerce-php-xampp)

---

### ⚙️ Programación paralela en Java
Simulación de procesos concurrentes con hilos y memoria compartida.  
**Tecnologías:** Java  
📂 [Ver proyecto](./projects/java-concurrencia)

---

### 📱 Aplicaciones educativas en Android
Aplicaciones móviles con inicio de sesión, Firebase y manejo de medios visuales.  
**Tecnologías:** Java, Android Studio, Firebase  
📂 [Ver proyecto](./projects/android-educativo)

---

### 🪟 Interfaces gráficas con Java (Swing)
Aplicaciones interactivas educativas con eventos y paneles dinámicos.  
**Tecnologías:** Java (Swing)  
📂 [Ver proyecto](./projects/java-swing-gui)

---

### 🧠 Gráficos 2D y 3D en Python
Visualizaciones gráficas para modelar conceptos de computación gráfica.  
**Tecnologías:** Python  
📂 [Ver proyecto](./projects/python-graficos-3d)

---

## 💡 Habilidades blandas
- Resolución de problemas y pensamiento crítico  
- Trabajo en equipo y comunicación efectiva  
- Adaptabilidad y resiliencia  
- Aprendizaje activo  
- Responsabilidad y ética profesional  

---

## 🌎 Idiomas
- Español: Nativo  
- Inglés: A2 – Básico (lectura técnica y comunicación simple)

---

## 📜 Educación

🎓 **Ingeniería en Desarrollo de Software**  
*CETI (Centro de Enseñanza Técnica Industrial)* — 2023 - presente  

🎓 **Tecnólogo en Desarrollo de Software**  
*CETI (Centro de Enseñanza Técnica Industrial)* — 2019 - 2022  

---

⭐ *Gracias por visitar mi perfil. Puedes seguir mis proyectos o contactarme para colaborar en nuevas ideas.*


<!--
**JulianEsp19/JulianEsp19** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
